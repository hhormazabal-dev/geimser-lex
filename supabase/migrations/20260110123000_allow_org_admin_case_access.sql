...
-- ---------------------------------------------------------------------------
-- Fix: allow org_admin/staff to see all cases in their active org.
-- This lets multi-role users (e.g. org admin + lawyer) view org cases.
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- Roles internos del org: acceso total dentro del org activo.
  IF EXISTS (
    SELECT 1
    FROM public.org_members m
    WHERE m.organization_id = v_case_org
      AND m.user_id = auth.uid()
      AND m.role IN ('org_admin'::public.org_member_role, 'staff'::public.org_member_role)
  ) THEN
    RETURN TRUE;
  END IF;
