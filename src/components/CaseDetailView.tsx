...
export function CaseDetailView({ case: caseData, profile }: CaseDetailViewProps) {
  const [activeTab, setActiveTab] = useState<'overview' | 'timeline' | 'documents' | 'notes' | 'requests' | 'clients'>('overview');
  const router = useRouter();

  const canEdit = profile.role === 'admin_firma' || 
  ...
  const getPriorityBadge = (priority: string) => {
    ...
    return (
      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${variants[priority] || 'bg-gray-100 text-gray-800'}`}>
            ...
            <div className='flex items-start justify-between mb-6'>
              <div className='flex-1'>
                ...
                <div className='flex items-center space-x-4 text-sm text-gray-600 mb-4'>
                  ...
                  {caseData.materia && (
                    <span>Materia: {caseData.materia}</span>
                  )}
                  {caseData.tribunal && (
                    <span className='flex items-center'>
              ...
              <div className='bg-purple-50 rounded-lg p-4'>
                ...
                <div className='space-y-1 text-sm text-purple-700'>
                  {caseData.fecha_inicio && (
                    <p>Inicio: {formatDate(caseData.fecha_inicio)}</p>
                  )}
                  {caseData.fecha_termino && (
                    <p>Término: {formatDate(caseData.fecha_termino)}</p>
                  )}
                </div>
              </div>
        ...
        <div>
          ...
          {activeTab === 'clients' && canManageClients && (
            <div className='space-y-6'>
              <MagicLinkManager 
                caseId={caseData.id}
                canManageLinks={canManageClients}
              />
